...
public:
    ...
    bool vertexGoesTo(Vertex<V, E> & a, Vertex<V, E> & b) {
        for (int i = 0; i < a.getOutgoingEdges()->size(); ++i) {
            Edge<V, E> currentEdge = *(*a.getOutgoingEdges())[i];
            if (*currentEdge.getDestination()->getInfo() == *b.getInfo()) {
                return true;
            }
        }
        return false;
    ...
    bool isMesh() {
        for (int i = 0; i < getVertices()->size(); ++i) {
            for (int j = 0; j < getVertices()->size(); ++j) {
                if (j == i) continue;
                if (!vertexGoesTo(*getVertexAt(i), *getVertexAt(j)) || !vertexGoesTo(*getVertexAt(j), *getVertexAt(i))) {
                    return false;
                }
            }
        }
    ...
    void connectAll() {
        for (int i = 0; i < getVertices()->size(); ++i) {
            Vertex<V, E> * currentVertex = (*getVertices())[i];
            for (int j = 0; j < getVertices()->size(); ++j) {
                if (j == i) continue;
                try {
                    E einfo;
                    addEdge(einfo, *currentVertex->getInfo(), *(*getVertices())[j]->getInfo());
                } catch (const char * exception) {
                    throw exception;
                }
            }
