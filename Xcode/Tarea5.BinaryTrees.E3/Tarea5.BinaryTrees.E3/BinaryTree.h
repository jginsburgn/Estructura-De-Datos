    ...
    bool BinaryTree<T>::isSymmetrical() const{
        if (!empty()) {
            if (root->getLeft() && root->getRight()) {
                BinaryTree<T> * leftSubtree = newSubtree(root->getRight());
                BinaryTree<T> * rightSubtree = newSubtree(root->getLeft());
                rightSubtree->reflect();
                std::vector<T> * l = new std::vector<T>();
                std::vector<T> * r = new std::vector<T>();
                
                std::vector<void *> argsl;
                argsl.push_back(l);
                std::vector<void *> argsr;
                argsr.push_back(r);
                
                leftSubtree->preOrder([](BNode<T> * node, std::vector<void *> args){
                    ((std::vector<T> *)args[0])->push_back(node->getInfo());
                }, argsl);
                rightSubtree->preOrder([](BNode<T> * node, std::vector<void *> args){
                    ((std::vector<T> *)args[0])->push_back(node->getInfo());
                }, argsr);
